...
class R2Net(nn.Module):
    ...
    def forward(self, x, prior=None):
        ...
        global_pre = F.upsample(global_pre, size=x.size()[2:], mode='bilinear', align_corners=True)
        pre0 = F.upsample(pre0, size=x.size()[2:], mode='bilinear', align_corners=True)
        pre1 = F.upsample(pre1, size=x.size()[2:], mode='bilinear', align_corners=True)
        pre2 = F.upsample(pre2, size=x.size()[2:], mode='bilinear', align_corners=True)
