...
class PoolNet(nn.Module):
    def __init__(self, base_model_cfg, base, convert_layers, deep_pool_layers, score_layers):
        ...
        self.up_feature = nn.Sequential(nn.Conv2d(2, 2, 1), nn.BatchNorm2d(2), nn.ReLU(inplace=True))
    ...
    def forward(self, x, prior=None):
        ...
        if prior is None:
            ...
            conbine = self.up_feature(torch.cat([merge, prior], dim=1))
            conbine_pred1 = F.sigmoid(conbine.narrow(1, 0, 1))

            merge = merge * conbine_pred1

            return merge

def build_model_poolnet(base_model_cfg='vgg'):
    if base_model_cfg == 'vgg':
